...
def abrir_planilha_e_exibir():
    ...
    if caminho_arquivo is not None:
        try:
            ...
            global dados
            dados = pd.read_excel(caminho_arquivo) if caminho_arquivo.name.endswith('xlsx') else pd.read_csv(caminho_arquivo)
...
def aplicar_metodo_estatistico(coluna, dados_filtrados):
    ...
    if metodo_estatistico.startswith("GroupBy"):
        ...
        if resultado_groupby is not None:
            ...
            modo_exibicao = st.selectbox("Selecione o modo de exibição do gráfico", ["BAR", "LINE", "SCATTER", "BOXPLOT"])
            # Exibir o gráfico
        ...
        resultado = aplicar_metodo_estatistico_individual(coluna, metodo_estatistico, dados_filtrados)
        # Exibir boxplot se o método for "Describe" e a coluna for numérica
        if metodo_estatistico == "Describe" and pd.api.types.is_numeric_dtype(dados_filtrados[coluna]):
            st.write("### Boxplot")
            fig = px.box(dados_filtrados, y=coluna)
            st.plotly_chart(fig)
        return resultado
...
def exibir_grafico(dados, coluna_x, coluna_y, modo_exibicao):
    if dados is not None:
        if modo_exibicao == "BAR":
            fig = px.bar(dados, x=coluna_x, y=coluna_y)
        elif modo_exibicao == "LINE":
            fig = px.line(dados, x=coluna_x, y=coluna_y)
        elif modo_exibicao == "SCATTER":
            fig = px.scatter(dados, x=coluna_x, y=coluna_y)
        elif modo_exibicao == "BOXPLOT":
            fig = px.box(dados, x=coluna_x, y=coluna_y)
